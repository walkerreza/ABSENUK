...
class ProfileController extends GetxController {
  ...
  final RxBool isLoading = false.obs; // Untuk proses save
  final RxBool isProfileLoading = true.obs; // Untuk proses load awal
  ...
  final List<int> semesters = [1, 2, 3, 4]; // Semester 1-4
  ...
  void onInit() {
    ...
    // Muat data awal jika ada
    final initialData = box.read<Map<String, dynamic>>('user');
    if (initialData != null) {
      _updateProfileData(initialData);
    } else {
      isProfileLoading.value = true;
    }

    // Dengarkan perubahan pada 'user' dan simpan fungsi cancel-nya
    _userListener = box.listenKey('user', (value) {
      if (value is Map<String, dynamic>) {
        _updateProfileData(value);
      }
    });
  }
  ...
  @override
  void onClose() {
    // Hentikan listener untuk mencegah memory leak dan crash
    _userListener();
    
    // Dispose controllers untuk melepaskan resource
  ...
  void _updateProfileData(Map<String, dynamic> userData) {
    isProfileLoading.value = true;
    try {
      ...
      if (userData['prodi'] != null && userData['prodi'] is Map) {
        final prodiName = userData['prodi']['nama'] as String?;
        if (prodiName != null) {
      ...
      existingImageUrl.value = userData['image'] ?? '';
    } catch (e) {
  ...
  Future<void> saveProfile() async {
    ...
    try {
      ...
      request.fields['semester'] = selectedSemester.value.toString();
...
//kodingan mu gil ngisor, duwur pekku
...
//       // Jika ada gambar baru, proses embedding SEBELUM mengirim request
//       if (profileImage.value != null) {
//         final mlService = Get.find<MLService>();
//         final imageFile = File(profileImage.value!.path);
//         newEmbedding = await mlService.getEmbeddingFromFile(imageFile);

//         if (newEmbedding != null) {
//           // Kirim embedding sebagai string JSON ke server
//           request.fields['face_embedding'] = jsonEncode(newEmbedding);
...
//           );
//         } else {
//           Get.snackbar('Gagal', 'Tidak dapat memproses fitur wajah dari gambar. Coba foto lain yang lebih jelas.', snackPosition: SnackPosition.BOTTOM);
//           isLoading.value = false;
//           return; // Hentikan proses jika embedding gagal
//         }
//       }
...
//         // Ambil data pengguna saat ini dari storage untuk referensi
//         final Map<String, dynamic> oldUserData = box.read('user') ?? {};
        
//         // Perbarui embedding di data lokal HANYA jika ada embedding baru yang berhasil dibuat.
//         // Ini memastikan kita menyimpan List<double>, bukan string dari server.
//         if (newEmbedding != null) {
//           updatedUserData['face_embedding'] = jsonEncode(newEmbedding);
//         } else {
//           // Jika tidak ada gambar baru, pertahankan embedding lama dari storage lokal.
//           updatedUserData['face_embedding'] = oldUserData['face_embedding'];
//         }

//         // Simpan data pengguna yang sudah diperbarui (dengan embedding yang benar) ke storage lokal
//         await box.write('user', updatedUserData);
